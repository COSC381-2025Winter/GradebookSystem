...
class Gradebook:
    ...
    def add_grade(self, instructor, course_id, student_id, grade):
        ...
        if not instructor.has_access(course_id):
            print("Access Denied: You are not authorized to grade this course.")
            return
        ...
        if student_id in self.grades[course_id]:
            print("Error: Grade already exists. Use 'edit' instead.")
        else:
            self.grades[course_id][student_id] = {"grade": grade, "timestamp": now}
            print(f"Grade added for student {student_id}: {grade}")
    ...
    def edit_grade(self, instructor, course_id, student_id, new_grade):
        ...
        if not instructor.has_access(course_id):
            print("Access Denied: You are not authorized to edit this course.")
            return
        ...
        if course_id in self.grades and student_id in self.grades[course_id]:
            ...
            if delta.days <= 7:
                ...
                print(f"Grade updated for student {student_id}: {new_grade}")
            else:
                print("Error: Grade editing period (7 days) has expired.")
        else:
            print("Error: No existing grade found. Use 'add' instead.")

    def view_grades(self, instructor, course_id):
        """Displays all grades for a course if the instructor is authorized"""
        if not instructor.has_access(course_id):
            print("Access Denied: You are not authorized to view this course.")
            return
